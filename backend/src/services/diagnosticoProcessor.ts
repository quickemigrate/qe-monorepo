...
import { obtenerContextoLegal } from './rag';

const anthropic = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY || '' });
const resend = new Resend(process.env.RESEND_API_KEY || '');

export function normalizarObjetivo(obj: string): string {
...
export function sanitizarScoring(texto: string): string {
...
export async function procesarConRetry(id: string, data: any, intento = 1): Promise<void> {
  try {
...
export async function procesarDiagnostico(diagnosticoId: string, data: any) {
...
Sé específico, útil y directo. Máximo 800 palabras en total. Sé directo y específico.`;
  ...
  if (email) {
    await resend.emails.send({
      ...
      `,
      attachments: [{
        filename: `diagnostico-quick-emigrate-${(data.nombre || 'cliente').toLowerCase().replace(/\s/g, '-')}.pdf`,
...
export async function generarPDF(nombre: string, informe: string, data: any): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    for (let i = 0; i < lineas.length; i++) {
      ...
      if (trimmed.startsWith('[SECCION]')) {
        ...
        doc.fillColor('#1A1C1C').font('Helvetica-Bold').fontSize(13)
        ...
        ensureSpace(40);
        doc.moveDown(0.3);
        doc.fillColor('#25D366').font('Helvetica-Bold').fontSize(10)
        ...
        const itemTextHeight = doc.heightOfString(item, { width: contentWidth - 22, lineGap: 2 });

        // Bullet + texto deben quedar en la misma página
        ensureSpace(itemTextHeight + 8);

        const currentY = doc.y;
        doc.circle(margin + 8, currentY + 5, 3).fillColor('#25D366').fill();
        doc.fillColor('#374151').font('Helvetica').fontSize(10)
          .text(item, margin + 22, currentY, { width: contentWidth - 22, lineGap: 2 });
        doc.y = currentY + itemTextHeight + 6;

      } else if (trimmed.startsWith('[TEXTO]')) {
        const texto = trimmed.replace('[TEXTO]', '').trim();
        doc.font('Helvetica').fontSize(10);
        const textoHeight = doc.heightOfString(texto, { width: contentWidth, lineGap: 3 });
        ensureSpace(textoHeight + 8);
        doc.fillColor('#374151').font('Helvetica').fontSize(10)
        ...
        const alertaTextHeight = doc.heightOfString(alerta, { width: contentWidth - 24, lineGap: 2 });
        const boxHeight = Math.max(50, alertaTextHeight + 32);
        ensureSpace(boxHeight + 8);

        const alertY = doc.y;
        doc.roundedRect(margin, alertY, contentWidth, boxHeight, 4).fill('#FEF3C7');
        doc.rect(margin, alertY, 4, boxHeight).fill('#F59E0B');
        ...
        doc.fillColor('#25D366').font('Helvetica-Bold').fontSize(26)
          .text(dest, margin + 16, destY + 12, { width: contentWidth - 32, lineGap: 4 });
