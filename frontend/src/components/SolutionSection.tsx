      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {[
          ...
          <motion.div
            ...
            className="bg-surface-container-low p-10 rounded-3xl"
          >
            <div className="w-16 h-16 bg-primary-container rounded-2xl flex items-center justify-center text-on-background mb-8">
              {item.icon}
            </div>
            <h3 className="text-2xl font-bold text-on-background mb-4">{item.title}</h3>
            <p className="text-on-background/60 leading-relaxed font-medium">{item.text}</p>
          </motion.div>
        ))}
      </div>
    </div>
