  ...
  return (
    <AdminLayout>
      <div className="p-8">
        ...
        {/* Planes */}
        <div className="mt-8 bg-white rounded-2xl border border-black/5 p-6 max-w-[900px]">
          <h2 className="text-[16px] font-semibold text-on-background mb-5">Gestión de Planes</h2>

          {loadingPlanes ? (
            <div className="text-[14px] text-on-background/40">Cargando planes...</div>
          ) : (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {planes.map((plan, idx) => (
                  <div key={plan.id} className="border border-black/8 rounded-xl p-4 space-y-3">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-[12px] font-bold uppercase tracking-widest text-on-background/40">{plan.id}</span>
                      <label className="flex items-center gap-2 cursor-pointer">
                        <span className="text-[12px] text-on-background/50">Activo</span>
                        <input
                          type="checkbox"
                          checked={plan.activo}
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1.5">
                        <label className={labelCls.replace('mb-1.5', '')}>Features</label>
                        <button
                      ...
                      <div className="space-y-1.5">
                        {plan.features.map((feat, fi) => (
                          <div key={fi} className="flex items-center gap-2">
                            ...
                            />
                            <button onClick={() => removeFeature(idx, fi)} className="shrink-0 text-red-400 hover:text-red-600">
                              <X size={14} />
                            </button>
                          </div>
                        ))}
              ...
              <button
                ...
                className="rounded-xl bg-on-background text-white font-semibold px-6 py-3.5 text-[15px]
                           hover:opacity-90 active:scale-[0.98] transition disabled:opacity-50"
              >
